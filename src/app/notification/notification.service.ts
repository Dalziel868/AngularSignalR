...
export class NotificationService {
  ...
  private notifyBaseUrl=environment.baseUrl+"api/notifications";
  constructor(private http:HttpClient) { }
  getNotificationCount():Observable<NotificationCountResult>
  {
    const url=`${this.notifyBaseUrl}/notificationcount`;
    return this.http.get<NotificationCountResult>(url)
    .pipe(catchError(this.handleError));
  }
  getNotificationMessage():Observable<Array<NotificationResult>>
  {
    const url=`${this.notifyBaseUrl}/notificationresult`;
      return this.http.get<Array<NotificationResult>>(url)
      .pipe(catchError(this.handleError));
  }
  deleteNotifications():Observable<{}>
  {
    const url=`${this.notifyBaseUrl}/deletenotifications`;
    const header=new HttpHeaders({'Content-Type':'application/json'});
    return this.http.delete(url,{headers:header}).pipe(catchError(this.handleError));
